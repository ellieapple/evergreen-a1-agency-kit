...
function callAI(prompt: string, maxTokens: number = 5000): Promise<string> {
...
export async function analyzeSERP(keyword: string, location?: string): Promise<string> {
...
export async function generateSEOBrief(keyword: string, business: BusinessContext, serpData: string, pageType: string): Promise<SEOBrief> {
...
RULES: Title keyword FIRST. Schema exampleJson must have REAL values. headingStructure must have REAL heading text. contentGaps must be SPECIFIC. Return ONLY valid JSON.`;
  ...
  return parseJSON<SEOBrief>(raw);
}

export async function generateKeywordCluster(seedKeyword: string, business: BusinessContext): Promise<KeywordData[]> {
...
Return ONLY valid JSON array.`;
  ...
  return parseJSON<KeywordData[]>(raw);
}

export async function runSiteAudit(url: string, business: BusinessContext): Promise<SiteAuditReport> {
